...
const ViewPosts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios.get('http://ec2-54-164-128-22.compute-1.amazonaws.com:3001/api/private/posts')
  ...
  const sendUpdateRequest = async (ratedPost, updatedPost) => {
    try {
      ...
      await axios.put('http://ec2-54-164-128-22.compute-1.amazonaws.com:3001/api/private/posts/' + ratedPost._id, updatedPost, {headers: {'authorization': token}})
  ...
  const handleRemove = async (post) => {
    ...
    try{
      const token = localStorage.getItem('authToken')
      await axios.delete('http://ec2-54-164-128-22.compute-1.amazonaws.com:3001/api/private/posts/' + postId, {headers: {'authorization': token}})
  ...
  return (
    <div className='view-posts'>
      ...
      {posts.map((post, idx) => (
        <div key={idx}>
          <div className='card'>
            <div className='card-header'><img src={`http://ec2-54-164-128-22.compute-1.amazonaws.com:3001/public/uploads/${post.setupImage}`} alt='setupImage'/></div>
